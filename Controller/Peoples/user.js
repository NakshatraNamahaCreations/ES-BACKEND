...
class User {
  async userfirebaseLogin(req, res) {
    ...
    try {
      ...
      if (!user) {
        return res.status(401).json({ error: "user not exit " });
      }
  ...
  async userfirebasesendotp(req, res) {
    const { phoneNumber } = req.body;
  ...
  async userfirebaseverifyotp(req, res) {
    ...
    try {
      ...
      // OTP is valid, proceed to navigate to /asin-code
...
async getUserDetails(req, res) {
    ...
    try {
      ...
      res.status(200).json({
        ...
        name: user.name,
        email: user.email,
        usedCoupons: user.usedCoupons, // Return used coupons
      });
    } catch (error) {
  ...
  async updateUsersearchcount(req, res) {
    try {
      const { id } = req.params; // Get the user ID from the request params
      const { searchLimit } = req.body; // Get the new search count from the request body
      ...
      // Update only the searchcount field
      const updatedUser = await userSchema.findOneAndUpdate(
        { _id: id },
        { searchLimit }, // Only update the searchcount field
      ...
      console.error("Error updating searchcount:", error);
      res.status(500).json({ error: error.message });
    }
  }
